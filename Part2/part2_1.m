...
%this script was used for the simulations in part 2-1 with only additive
%noise
        ...
        for j = 1:resolution
            ...
            for j = 1:rep_number
                if num_bits > 1
                    ...
                    bits_out_temp(j,:) = decode(encoding(2*j-1:2*j), P, num_bits, rep_number);
                else
                    %channel noise
                    n = normrnd(0,sigma);
                    encoding(j) = encoding(j) +  n;
                    %decoding the symbols to bits
                    bits_out_temp(j,:) = decode(encoding(j), P, num_bits, rep_number);
                end
            end
...
function bits = decode(codeword, P, num_bits, rep_number)
    if num_bits == 1
