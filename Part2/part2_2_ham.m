...
%this script was used for the simulations in part 2-2 with only additive
%noise

clear all;

num_bits = 6; 
rep_number = 7/4; %used as a scaling parameter here 
resolution = 100;
    ...
    for sigma = sig_lim(1):sig_lim(2):sig_lim(3) %varying sigma
        ...
        for l = 1:resolution
            for k=1:num_bits % number of grouped transmissions 
                ...
                input_bits(((k-1)*4+1):k*4) = bits(1:4);
                n = 7;
                k_ = 4;
                %encode the message to hamming code
                transmit(((k-1)*7+1):k*7) = encode(bits,n,k_,'hamming/binary');
            end
            num_coms = length(transmit) / num_bits; 
            for j=1:num_coms
                if num_bits > 1
                    %encode the hamming code to symbols
                    encoding(2*j-1:2*j) = encode_channel(transmit((j-1)*num_bits+1:j*num_bits), P, num_bits, rep_number);
                    %channel noise
                    n(1) = normrnd(0,sigma);
                    encoding(2*j-1) = encoding(2*j-1) +  n(1);
                    n(2) = normrnd(0,sigma);
                    encoding(2*j) = encoding(2*j) +  n(2);
                    %decoding the symbols to bits
                    bits_out_temp((j-1)*num_bits+1:j*num_bits) = decode_channel(encoding(2*j-1:2*j), P, num_bits, rep_number);
                else
                    %encode the hamming code to symbols
                    encoding(j) = encode_channel(transmit((j-1)*num_bits+1:j*num_bits), P, num_bits, rep_number);
                    %channel noise
                    n = normrnd(0,sigma);
                    encoding(j) = encoding(j) +  n;
                    %decoding the symbols to bits
                    bits_out_temp((j-1)*num_bits+1:j*num_bits) = decode_channel(encoding(j), P, num_bits, rep_number);
                end
            end
            for j = 1:(length(bits_out_temp)/7)
                %decode the output hamming message
                n=7;
                k=4;
                bits_out = decode(bits_out_temp(((j-1)*7+1):j*7),n,k,'hamming/binary');
                total = total + 4;
                bit_check = input_bits(((j-1)*4+1):j*4);
                for i=1:4
...
function bits = decode_channel(codeword, P, num_bits, rep_number)
    if num_bits == 1
