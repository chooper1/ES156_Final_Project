...
for k=1:size(B2,1)/8
    for l=1:size(B2,2)/8  
        ...
        for i=1:4 %to get around type errors
            if bits(i) == 1
                bitvec(i) = 1;
            else
                bitvec(i) = 0;
        ...
        n_ = 7;
        k_ = 4;
        transmit = encode(bitvec,n_,k_,'hamming/binary');
        transmit(8) = 0; % padding the message
        %transmit the coefficients
        for i = 1:2 % two 4-bit transmissions
            for j=1:5 %4 dct bit values 
                %encoding the message as symbols
                encoding = encode_channel(transmit((i-1)*4+1:i*4), P, num_bits, rep_number);
                %channel noise
                n(1) = normrnd(0,sigma);
                n(2) = normrnd(0,sigma);
                h(1) = normrnd(0,1); 
