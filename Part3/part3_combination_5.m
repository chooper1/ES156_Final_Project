...
for k=1:size(B2,1)/8
    for l=1:size(B2,2)/8  
        ...
        n_ = 7;
        k_ = 4;
        for i=1:3 %repetition code
            transmit((i-1)*8+1:8*i-1) = encode(bitvec((i-1)*4+1:4*i),n_,k_,'hamming/binary');
            transmit(8*i) = 0; %padding the messages 
        end
        
        %transmit the coefficients
        for i = 1:4 % 4 6-bit transmissions
            for j=1:5 % repetition code w/ K=5
                
                %encoding the message as symbols
                encoding = encode_channel(transmit((i-1)*6+1:i*6), P, num_bits, rep_number);
                %channel noise
                n(1) = normrnd(0,sigma);
                n(2) = normrnd(0,sigma);
                h(1) = normrnd(0,1); 
                ...
                bits_out_temp(i,(j-1)*6+1:j*6)  = decode_channel(encoding, P, num_bits, rep_number, h); 
            end
        end
        %recovering the repeated transmissions and using majority rule
        for j=1:4 % 4 transmissions
            bits_out = bits_out_temp(j,:);
            for i=1:6 %6 bit transmissions
                count = 0;
                if(bits_out(i) == 1)
                    count = count + 1;
                end
        ...
        %decoding the hamming code to the output message
        bits_out = decode(bits_maj(1:7),n_,k_,'hamming/binary');
        int_out = bi2de(bits_out);
        B3((k-1)*8+1,(l-1)*8+1) = int_out;
        
        %decoding the hamming code to the output message
        bits_out = decode(bits_maj(9:15),n_,k_,'hamming/binary');
        int_out = bi2de(bits_out);
        B3((k-1)*8+2,(l-1)*8+1) = int_out;
        
        %decoding the hamming code to the output message
        bits_out = decode(bits_maj(17:23),n_,k_,'hamming/binary');
        int_out = bi2de(bits_out);
