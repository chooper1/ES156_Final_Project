...
for k=1:size(B2,1)/8
    for l=1:size(B2,2)/8  
        ...
        k_ = 4;
        for i = 1:3
            %encoding the message using hamming code
            transmit = encode(bitvec((i-1)*4+1:4*i),n_,k_,'hamming/binary');
            for j=1:7 %4 dct bit values
                %encoding the message to symbols
                encoding(j) = encode_channel(transmit(j), P, num_bits, rep_number);
                %channel noise
                n = normrnd(0,sigma);
                h = normrnd(0,1);
                encoding(j) = h*encoding(j) +  n;
                %decoding the symbols to bits
                bits_rec(j) = decode_channel(encoding(j), P, num_bits, rep_number, h);
            end
            %decoding the hamming code to the output message
            bits_out((i-1)*4+1:4*i) = decode(bits_rec,n_,k_,'hamming/binary');
        end
        
        %set 8 by 8 block using the recovered frequency components
        int_out = bi2de(bits_out(1:4));
        B3((k-1)*8+1:8*k,(l-1)*8+1:8*l) = zeros(8);
        B3((k-1)*8+1,(l-1)*8+1) = int_out;
        int_out = bi2de(bits_out(5:8));
        B3((k-1)*8+2,(l-1)*8+1) = int_out;
        int_out = bi2de(bits_out(9:12));
