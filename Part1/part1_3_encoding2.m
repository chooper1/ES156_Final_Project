...
%E_b == P/num_bits for part 1-3

%this script was used to simulate the second encoding strategy
...
for P=P_lim(1):P_lim(2):P_lim(3) %varying P
    for sigma = sig_lim(1):sig_lim(2):sig_lim(3) %varying sigma
        ...
        for j = 1:resolution
            ...
            n(1) = normrnd(0,sigma);
            n(2) = normrnd(0,sigma);
            encoding(1) = encoding(1) +  n(1);
            encoding(2) = encoding(2) +  n(2);
            
            %recovering the message from the channel and checking whether
            %it matches the input
            bits_out = decode(encoding, P, num_bits);
...
function codeword = encode(bits, P, num_bits)
    if num_bits == 4
        %scaling to meet average power constraints
        power_scaled = sqrt(P/10);
        %encode the message as symbols
...
%decoding function
%this function scales the power to determine the appropriate thresholds, 
%and then decodes the symbols into an output message
function bits = decode(codeword, P, num_bits)
    if num_bits == 4
        %scaling to meet average power constraints
        power_scaled = sqrt(P/10);
        %decode the symbols to bits
